"""
Generate sardonic one-liner descriptions for each bus in the fleet,
using the Bristol Bus Bot persona via Gemini 3 Pro.

Sends buses in batches of ~80 to stay well within context limits.
Outputs bus-descriptions.json keyed by fleet_code.

Usage: python generate_bus_descriptions.py
"""
...
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "bus-descriptions.json")

BATCH_SIZE = 80  # buses per API call

# Condensed model blurbs giving Gemini actual knowledge about each bus type
MODEL_BLURBS = {
...
Your task: write a SHORT one-liner description (max 15 words) for each bus. These appear in tooltips on a live bus map.

CRITICAL RULES:
- Max 15 words each. Shorter is better. Aim for 8-12 words.
- ALWAYS reference what type of bus it is — the model is the most interesting thing about each bus. Is it a tiny midibus? A premium double-decker with a glass staircase? A Chinese-built electric? An ancient Volvo? A rare hybrid? USE this.
- DO NOT default to livery/branding jokes. Only mention livery if it's genuinely absurd (e.g. Essex livery on a Bath bus, AirportFlyer branding on standard service). Most livery names are boring and not worth mentioning.
- Reference specific technical details when they're funny (glass staircase, autonomous trials, biogas, coach-style interiors on a city bus, vibration-prone StreetLites)
- Electric buses get grudging respect — acknowledge the progress
- Ancient diesels get cold observation about age and emissions
- Coaches doing city routes get noted for being out of place
- USB ports on otherwise terrible buses are worth a mention
- Vary your angles: age, technology, fuel, size, purpose, features, depot, absurdity
- No emojis. No hashtags. British spelling.
- Don't be repetitive — each description must take a different angle from the others
...
- "Enviro400 City. glass staircase, standard delays. the luxury experience."
- "Yutong E12. electric, Chinese-built, actually works. suspicious."
- "StreetLite DF. 10 metres of vibration therapy."
- "2007 Gemini. older than the fare cap promise."
- "biogas double-decker. runs on compressed methane. smells like it too."
- "Enviro200. did autonomous trials once. now just autonomously late."
- "Plaxton Panther coach doing city work. how the mighty have fallen."

Return a JSON object mapping each fleet_code to its description. ONLY valid JSON, no markdown, no code fences."""
...
def load_fleet():
    ...
    # Filter to non-withdrawn buses with useful data
...
def summarise_bus(bus):
    """Create a compact summary of a bus for the prompt, leading with model info."""
    ...
    # Lead with the model name so descriptions remain useful in compact views.
    parts.append(f"MODEL: {model_name}")

    # Model context from blurbs — gives Gemini real knowledge about this bus type
    ...
    # Secondary details (livery de-emphasised)
    if garage.get('name'):
        parts.append(f"depot: {garage['name']}")
    if features:
        parts.append(f"features: {', '.join(features)}")
    if bus.get('reg'):
        parts.append(f"reg: {bus['reg']}")
    if livery.get('name'):
        parts.append(f"livery: {livery['name']}")
    if bus.get('branding'):
        parts.append(f"branding: {bus['branding']}")
...
def call_gemini(bus_summaries, batch_label=""):
    ...
    prompt = f"""Here are the buses. Write a one-liner for each.

{json.dumps(bus_summaries, indent=None)}

Return a JSON object mapping fleet_code to description. ONLY JSON, nothing else."""
    ...
    # Parse JSON from response — handle common Gemini formatting issues
    text = text.strip()
    if text.startswith('```'):
        text = re.sub(r'^```\w*\n?', '', text)
        text = re.sub(r'\n?```$', '', text)

    # Fix "Extra data" errors: Gemini sometimes outputs multiple JSON objects
    # or trailing garbage after the closing brace. Find the matching closing brace.
    try:
        ...
        # Last resort: fix unterminated strings by closing them
...
def process_batch(batch_num, total_batches, batch_summaries):
    """Process a single batch — called in parallel by ThreadPoolExecutor."""
    ...
    try:
        ...
        if len(descriptions) < len(batch_summaries):
            missing = set(batch_summaries.keys()) - set(descriptions.keys())
            print(f"  [{label}] WARNING: {len(missing)} buses missing from response: {list(missing)[:5]}...")
        return descriptions
...
def main():
    ...
    # Load existing descriptions to allow resuming
    existing = {}
    if os.path.exists(OUTPUT_PATH):
        with open(OUTPUT_PATH, 'r', encoding='utf-8') as f:
            existing = json.load(f)
        print(f"Loaded {len(existing)} existing descriptions (will skip these)")

    # Filter out already-described buses
    ...
    print(f"{len(todo)} buses need descriptions")

    if not todo:
        print("All buses already described!")
        return
    ...
    print(f"\nDone! {len(existing)} bus descriptions saved to {OUTPUT_PATH}")
