...
# Adapted from bustimes.org's TransXChange parser:
#   https://github.com/bustimes/bustimes.org
# See THIRD_PARTY_NOTICES.md at the repository root.
#
# Local changes: the two Django dependencies are replaced by local
# equivalents. Route geometry is not used for punctuality matching, so the
# GEOS types are harmless stubs.
...
def parse_duration(value):
    """Parse an ISO 8601 duration (PT1M30S, PT300S, PT2H, P1D...) to a timedelta.
    TransXChange RunTime / WaitTime use this form."""
    if not value:
        return None
    match = re.match(
...
class JourneyPattern:
    ...
    def __init__(self, element, sections, serviced_organisations):
        ...
        if (
            ...
            # Clockwise and anticlockwise values are not handled here.
    ...
    def __init__(self, element, stops):
        ...
        if self.wait_time is not None:
            ...
            if self.wait_time.total_seconds() > 10000:
                # Implausible wait time in the source data; log it and carry on.
                logger.warning(
