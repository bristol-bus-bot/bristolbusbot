"""
Generate cute depot-specific one-liner descriptions for each bus,
using the Bristol Bus Bot persona via Gemini 3 Pro.

These appear when a bus is parked at a depot — a different vibe from
the in-service descriptions. Sleeping buses, resting buses, waiting buses.

Outputs depot-descriptions.json keyed by fleet_code.

Usage: python generate_depot_descriptions.py
"""
...
OUTPUT_PATH = os.path.join(os.path.dirname(__file__), "depot-descriptions.json")

BATCH_SIZE = 80

# Same model blurbs from the main description generator
MODEL_BLURBS = {
...
SYSTEM_PROMPT = """You are the Bristol Bus Bot. You document Bristol's privatised bus network with warmth underneath the dry wit. These descriptions appear on a live bus map when a bus is parked at a depot — not in service.

Your task: write a SHORT cute/funny depot one-liner (max 15 words) for each bus. These show when the bus is sleeping/resting/charging at a depot.

CRITICAL RULES:
- Max 15 words each. Shorter is better. Aim for 8-12 words.
- The tone is FOND and WARM but still dry. These buses are resting. Be gentle.
- Reference what the bus IS — electric buses are charging, old diesels are recovering, tiny midibuses are hiding, coaches are sulking about doing city routes
- Play on the idea of buses having feelings: sleeping, dreaming, resting, plotting, sulking, hiding, charging, brooding, waiting
- Electric buses: they're plugged in, drinking electrons, charging up for another day of being the future
- Old diesels: creaking, complaining, dreaming of retirement, resting their weary chassis
- Biogas buses: digesting, fermenting, doing their weird compressed methane thing
- Double-deckers: looming over the smaller buses, taking up too much space
- Tiny midibuses: hiding behind the big buses, pretending to be invisible
- Coaches doing city work: wondering where it all went wrong
- Reference the depot if it adds comedy (Lawrence Hill = gritty, Hengrove = the new EV palace, Bath = the island, Weston = seaside exile)
- No emojis. No hashtags. British spelling.
- Don't be repetitive — each description must take a different angle
- These should make someone smile. Think Pixar-meets-public-transport.

Examples of the right tone:
- "Charging up. Tomorrow: same routes, same delays, better conscience."
- "Asleep at Lawrence Hill. Dreaming of routes that run on time."
- "Plugged in at Hengrove. £44 million depot, same 4-minute delays."
- "Resting its 20-year-old chassis. Earned it, frankly."
- "Hiding behind a double-decker. Classic Solo move."
- "Biogas Scania, sleeping it off. Even the methane needs a break."
- "Coach. Parked up. Still wondering how it ended up on the 39."

Return a JSON object mapping each fleet_code to its depot description. ONLY valid JSON, no markdown, no code fences."""
...
def call_gemini(bus_summaries, batch_label=""):
    ...
    prompt = f"""Here are the buses. Write a cute depot one-liner for each — they're parked up and resting.

{json.dumps(bus_summaries, indent=None)}

Return a JSON object mapping fleet_code to depot description. ONLY JSON, nothing else."""
...
def main():
    ...
    if os.path.exists(OUTPUT_PATH):
        ...
        print(f"Loaded {len(existing)} existing depot descriptions (will skip these)")
    ...
    print(f"{len(todo)} buses need depot descriptions")

    if not todo:
        print("All buses already have depot descriptions!")
        return
    ...
    print(f"\nDone! {len(existing)} depot descriptions saved to {OUTPUT_PATH}")
