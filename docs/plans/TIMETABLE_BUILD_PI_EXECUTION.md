...
gate and was atomically installed, but the collector recovery probe exceeded
its original 15-second subprocess timeout on the Pi. The transaction correctly
restored the old database. The probe timeout was raised to 45 seconds with six
bounded attempts and a regression test; the transaction ceiling was raised to
20 minutes so rollback retains its own complete recovery window. Automatic
